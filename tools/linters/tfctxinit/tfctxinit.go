...
var Analyzer = &analysis.Analyzer{
	Name: "tfctxinit",
	Doc:  "Ensures that ctx = core.InitProviderContext(ctx) is called in every resource/datasource CRUD method.",
	Run:  run,
}
...
func run(pass *analysis.Pass) (any, error) {
	for _, file := range pass.Files {
		ast.Inspect(file, func(n ast.Node) bool {
			...
			// Scan the body for: ctxName = core.InitProviderContext(ctxName)
			found := false
			if fn.Body != nil {
				for _, stmt := range fn.Body.List {
					...
					// The function called must be core.InitProviderContext
					sel, ok := call.Fun.(*ast.SelectorExpr)
					if !ok || sel.Sel.Name != "InitProviderContext" {
						continue
					}
			...
			if !found {
				pass.Reportf(
					fn.Pos(), 
					"Terraform %s method must call %s = core.InitProviderContext(%s)", 
