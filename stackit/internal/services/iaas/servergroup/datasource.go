...
// serverGroupDataSourceBetaCheckDone is used to prevent multiple checks for beta resources.
// This is a workaround for the lack of a global state in the provider and
// needs to exist because the Configure method is called twice.
var serverGroupDataSourceBetaCheckDone bool
...
// NewServerGroupDataSource is a helper function to simplify the provider implementation.
func NewServerGroupDataSource() datasource.DataSource {
	return &serverGroupDataSource{}
}

// serverGroupDataSource is the data source implementation.
type serverGroupDataSource struct {
	client *iaasalpha.APIClient
}

// Metadata returns the data source type name.
func (d *serverGroupDataSource) Metadata(_ context.Context, req datasource.MetadataRequest, resp *datasource.MetadataResponse) {
...
func (d *serverGroupDataSource) Configure(ctx context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
	...
	if !serverGroupDataSourceBetaCheckDone {
		features.CheckBetaResourcesEnabled(ctx, &providerData, &resp.Diagnostics, "stackit_server_group", "data source")
		if resp.Diagnostics.HasError() {
			return
		}
		serverGroupDataSourceBetaCheckDone = true
	}
...
func (r *serverGroupDataSource) Schema(_ context.Context, _ datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	resp.Schema = schema.Schema{
		MarkdownDescription: features.AddBetaDescription("Server group datasource schema. Must have a `region` specified in the provider configuration."),
		Description:         "Server group datasource schema. Must have a `region` specified in the provider configuration.",
...
func (d *serverGroupDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) { // nolint:gocritic // function signature required by Terraform
	var model Model
	...
	serverGroupResp, err := d.client.GetServerGroup(ctx, projectId, serverGroupId).Execute()
	if err != nil {
