...
func (d *routingTableRoutesDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) { // nolint:gocritic // function signature required by Terraform
	...
	routesResp, _, err := d.client.DefaultAPI.ListRoutesOfRoutingTable(ctx, organizationId, networkAreaId, region, routingTableId).Execute()
	if err != nil {
		utils.LogError(
			ctx,
			&resp.Diagnostics,
...
func mapDataSourceRoutingTableRoutes(ctx context.Context, routes *iaas.RouteListResponse, model *RoutingTableRoutesDataSourceModel, region string) error {
	if routes == nil {
