...
var testConfigVarsMax = config.Variables{
	...
	"listener_protocol":                 config.StringVariable("PROTOCOL_TLS_PASSTHROUGH"),
	"network_role":                      config.StringVariable("ROLE_LISTENERS_AND_TARGETS"),
	"tcp_idle_timeout":                  config.StringVariable("42s"),
		...
		Steps: []resource.TestStep{
			// Creation
			{
				...
				Check: resource.ComposeAggregateTestCheckFunc(
					...
					resource.TestCheckResourceAttr("stackit_loadbalancer.loadbalancer", "listeners.0.tcp.idle_timeout", testutil.ConvertConfigVariable(testConfigVarsMax["tcp_idle_timeout"])),
					resource.TestCheckResourceAttrSet("stackit_loadbalancer.loadbalancer", "networks.0.network_id"),
			...
			// Data source
			{
				...
				Check: resource.ComposeAggregateTestCheckFunc(
					...
					resource.TestCheckResourceAttr("data.stackit_loadbalancer.loadbalancer", "listeners.0.tcp.idle_timeout", testutil.ConvertConfigVariable(testConfigVarsMax["tcp_idle_timeout"])),
					resource.TestCheckResourceAttrSet("data.stackit_loadbalancer.loadbalancer", "networks.0.network_id"),
