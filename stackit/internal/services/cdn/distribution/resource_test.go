...
func TestModifyPlan(t *testing.T) {
	...
	t.Run("update via waf removal when enabled list had prior state: enabled_rule_collection_ids becomes unknown", func(t *testing.T) {
		// state: disabled_rule_ids populated, enabled_rule_collection_ids carried over as empty set
		state := modifyPlanModel(tsOld, wafObjForModifyPlan("ENABLED", disabledPopulated, disabledEmpty))
		// plan: framework carried over enabled_rule_collection_ids empty set from state
		plan := modifyPlanModel(tsOld, wafObjForModifyPlan("ENABLED", disabledEmpty, disabledEmpty))
		// config: user removed disabled_rule_ids (null), enabled_rule_collection_ids unconfigured (null)
		config := modifyPlanModel(tsOld, wafObjForModifyPlan("ENABLED", nullSet, nullSet))
