...
const (
	testVersion        = "1.2.3"
	testCustomEndpoint = "https://valkey-custom-endpoint.api.stackit.cloud"
)
...
func TestConfigureClient(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			expected: func() *valkey.APIClient {
				apiClient, err := valkey.NewAPIClient(
		...
		},
		{
			...
			args: args{
				providerData: &core.ProviderData{
					Version:              testVersion,
					ValkeyCustomEndpoint: testCustomEndpoint,
				},
			},
			expected: func() *valkey.APIClient {
				apiClient, err := valkey.NewAPIClient(
