...
const (
	testVersion        = "1.2.3"
	testCustomEndpoint = "https://serverbackup-custom-endpoint.api.stackit.cloud"
)
...
func TestConfigureClient(t *testing.T) {
	...
	tests := []struct {
		...
		args     args
		wantErr  bool
		expected *serverbackup.APIClient
	}{
		{
			...
			expected: func() *serverbackup.APIClient {
		...
		},
		{
			...
			args: args{
				providerData: &core.ProviderData{
					Version:                    testVersion,
					ServerBackupCustomEndpoint: testCustomEndpoint,
				},
			},
			expected: func() *serverbackup.APIClient {
				apiClient, err := serverbackup.NewAPIClient(
