...
func TestAccRunCommandAction(t *testing.T) {
	...
	testConfigVars := config.Variables{
		...
		"machine_type":      config.StringVariable("s3i.2"),
...
func testAccCheckRunCommandExecuted(s *terraform.State) error {
	...
	client, err := runcommand.NewAPIClient(
		testutil.NewConfigBuilder().BuildClientOptions(testutil.RunCommandCustomEndpoint, false)...,
	)
	if err != nil {
	...
	for _, rs := range s.RootModule().Resources {
		...
		cmdsResp, err := client.DefaultAPI.ListCommands(ctx, projectId, serverId, region).Execute()
		if err != nil {
...
func testAccCheckNicDestroy(s *terraform.State) error {
	...
	for _, rs := range s.RootModule().Resources {
		...
		parts := strings.Split(rs.Primary.ID, core.Separator)
		if len(parts) < 4 {
			return fmt.Errorf("unexpected network interface ID format: %s", rs.Primary.ID)
		}
		projectId, region, networkId, nicId := parts[0], parts[1], parts[2], parts[3]

		_, err := client.DefaultAPI.GetNic(ctx, projectId, region, networkId, nicId).Execute()
		if err == nil {
			return fmt.Errorf("network interface %s still exists after destroy", nicId)
		}
	}
...
func testAccCheckNetworkDestroy(s *terraform.State) error {
	...
	for _, rs := range s.RootModule().Resources {
		...
		parts := strings.Split(rs.Primary.ID, core.Separator)
		if len(parts) < 3 {
			return fmt.Errorf("unexpected network ID format: %s", rs.Primary.ID)
		}
		projectId, region, networkId := parts[0], parts[1], parts[2]

		_, err := client.DefaultAPI.GetNetwork(ctx, projectId, region, networkId).Execute()
		if err == nil {
			return fmt.Errorf("network %s still exists after destroy", networkId)
		}
	}
