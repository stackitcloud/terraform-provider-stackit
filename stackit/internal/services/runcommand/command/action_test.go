...
func TestToCreatePayload(t *testing.T) {
	tests := []struct {
		...
		expected    *v2api.CreateCommandPayload
		isValid     bool
	}{
		{
		...
		},
		{
			...
			expected: func() *v2api.CreateCommandPayload {
				p := v2api.NewCreateCommandPayload("RunShellScript")
				return p
			}(),
			isValid: true,
		},
		...
		},
		{
			...
			expected: func() *v2api.CreateCommandPayload {
				p := v2api.NewCreateCommandPayload("RunPowerShellScript")
		...
		},
		{
			...
			expected: func() *v2api.CreateCommandPayload {
				p := v2api.NewCreateCommandPayload("")
				return p
			}(),
			isValid: true,
		},
		...
		t.Run(tt.description, func(t *testing.T) {
			...
			if tt.isValid {
				diff := cmp.Diff(output, tt.expected,
					cmpopts.IgnoreUnexported(v2api.CreateCommandPayload{}),
				)
				if diff != "" {
