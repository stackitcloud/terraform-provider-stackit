	...
	for _, rs := range s.RootModule().Resources {
		...
		if err != nil {
			...
			if errors.As(err, &oapiErr) {
				...
				// workaround: when the delete endpoint is called a second time for a wrapping key which is already
				// scheduled for deletion, one will get an HTTP 400 error which we have to ignore here
				if oapiErr.StatusCode == http.StatusBadRequest {
					continue
				}
			}
