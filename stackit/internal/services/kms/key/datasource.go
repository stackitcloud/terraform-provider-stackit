...
func (k *keyDataSource) Configure(ctx context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
	...
	apiClient := kmsUtils.ConfigureClient(ctx, &k.providerData, &resp.Diagnostics)
	...
	resp.Schema = schema.Schema{
		...
		Attributes: map[string]schema.Attribute{
			...
			"algorithm": schema.StringAttribute{
				Description: fmt.Sprintf("The encryption algorithm that the key will use to encrypt data. %s", utils.FormatPossibleValues(utils.SliceToStringSlice(kms.AllowedAlgorithmEnumValues)...)),
				Computed:    true,
				Validators: []validator.String{
					stringvalidator.LengthAtLeast(1),
				},
			...
			"purpose": schema.StringAttribute{
				Description: fmt.Sprintf("The purpose for which the key will be used. %s", utils.FormatPossibleValues(utils.SliceToStringSlice(kms.AllowedPurposeEnumValues)...)),
				Computed:    true,
				Validators: []validator.String{
					stringvalidator.LengthAtLeast(1),
				},
