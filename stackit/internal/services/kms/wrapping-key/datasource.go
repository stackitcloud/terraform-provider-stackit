	...
	response.Schema = schema.Schema{
		...
		Attributes: map[string]schema.Attribute{
			"access_scope": schema.StringAttribute{
				Description: fmt.Sprintf("The access scope of the key. Default is `%s`. %s", string(kms.ACCESSSCOPE_PUBLIC), utils.FormatPossibleValues(utils.SliceToStringSlice(kms.AllowedAccessScopeEnumValues)...)),
				Computed:    true,
			},
			"algorithm": schema.StringAttribute{
				Description: fmt.Sprintf("The wrapping algorithm used to wrap the key to import. %s", utils.FormatPossibleValues(utils.SliceToStringSlice(kms.AllowedWrappingAlgorithmEnumValues)...)),
				Computed:    true,
			},
			...
			"protection": schema.StringAttribute{
				Description: fmt.Sprintf("The underlying system that is responsible for protecting the key material. %s", utils.FormatPossibleValues(utils.SliceToStringSlice(kms.AllowedProtectionEnumValues)...)),
				Computed:    true,
			},
			"purpose": schema.StringAttribute{
				Description: fmt.Sprintf("The purpose for which the key will be used. %s", utils.FormatPossibleValues(utils.SliceToStringSlice(kms.AllowedWrappingPurposeEnumValues)...)),
				Computed:    true,
			},
