...
const (
	testVersion        = "1.2.3"
	testCustomEndpoint = "https://mariadb-custom-endpoint.api.stackit.cloud"
)
...
func TestConfigureClient(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			expected: func() *mariadb.APIClient {
		...
		},
		{
			...
			args: args{
				providerData: &core.ProviderData{
					Version:               testVersion,
					MariaDBCustomEndpoint: testCustomEndpoint,
				},
			},
			expected: func() *mariadb.APIClient {
				apiClient, err := mariadb.NewAPIClient(
