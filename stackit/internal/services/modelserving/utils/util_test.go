...
const (
	testVersion        = "1.2.3"
	testCustomEndpoint = "https://modelserving-custom-endpoint.api.stackit.cloud"
)
...
func TestConfigureClient(t *testing.T) {
	...
	tests := []struct {
		...
		args     args
		wantErr  bool
		expected *modelserving.APIClient
	}{
		{
			...
			expected: func() *modelserving.APIClient {
		...
		},
		{
			...
			args: args{
				providerData: &core.ProviderData{
					Version:                    testVersion,
					ModelServingCustomEndpoint: testCustomEndpoint,
				},
			},
			expected: func() *modelserving.APIClient {
				apiClient, err := modelserving.NewAPIClient(
