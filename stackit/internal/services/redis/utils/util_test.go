...
const (
	testVersion        = "1.2.3"
	testCustomEndpoint = "https://redis-custom-endpoint.api.stackit.cloud"
)
	...
	tests := []struct {
		...
		},
		{
			...
			args: args{
				providerData: &core.ProviderData{
					Version:             testVersion,
					RedisCustomEndpoint: testCustomEndpoint,
				},
			},
			expected: func() *redis.APIClient {
				apiClient, err := redis.NewAPIClient(
