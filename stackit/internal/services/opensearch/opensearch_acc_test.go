...
var testConfigVarsMinUpdated = func() config.Variables {
	updatedConfig := make(config.Variables, len(testConfigVarsMin))
...
var testConfigVarsMax = config.Variables{
	...
	"observability_instance_plan_name": config.StringVariable("Observability-Monitoring-Starter-EU01"),
}

var testConfigVarsMaxUpdated = func() config.Variables {
	updatedConfig := make(config.Variables, len(testConfigVarsMax))
	...
	updatedConfig["plugin"] = config.StringVariable("")
		...
		Steps: []resource.TestStep{
			// Creation
			{
				...
				Check: resource.ComposeAggregateTestCheckFunc(
					...
					resource.TestCheckResourceAttr("stackit_opensearch_instance.instance", "parameters.plugins.#", "1"),
					resource.TestCheckResourceAttr("stackit_opensearch_instance.instance", "parameters.plugins.0", testutil.ConvertConfigVariable(testConfigVarsMax["plugin"])),
			...
			// Data source
			{
				...
				Check: resource.ComposeAggregateTestCheckFunc(
					...
					resource.TestCheckResourceAttr("data.stackit_opensearch_instance.instance", "parameters.plugins.#", "1"),
					resource.TestCheckResourceAttr("data.stackit_opensearch_instance.instance", "parameters.plugins.0", testutil.ConvertConfigVariable(testConfigVarsMax["plugin"])),
			...
			// Update
			{
				...
				Check: resource.ComposeAggregateTestCheckFunc(
					...
					resource.TestCheckResourceAttr("stackit_opensearch_instance.instance", "parameters.plugins.#", "0"),
