...
var testConfigVarsMaxUpdated = func() config.Variables {
	...
	updatedConfig["plugin"] = config.StringVariable("") // TODO: change this value to a valid value
		...
		Steps: []resource.TestStep{
			...
			// Data source
			{
				...
				Config: fmt.Sprintf(`
					...
					testutil.OpenSearchProviderConfig(), resourceMaxConfig,
				),
				Check: resource.ComposeAggregateTestCheckFunc(
					// Instance data
			...
			// Update
			{
				...
				Check: resource.ComposeAggregateTestCheckFunc(
					...
					resource.TestCheckResourceAttr("stackit_opensearch_instance.instance", "parameters.plugins.#", "1"),
					resource.TestCheckResourceAttr("stackit_opensearch_instance.instance", "parameters.plugins.0", testutil.ConvertConfigVariable(testConfigVarsMaxUpdated["plugin"])),
