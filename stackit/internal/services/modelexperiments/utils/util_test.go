...
const (
	testVersion        = "1.2.3"
	testCustomEndpoint = "https://modelexperiments-custom-endpoint.api.stackit.cloud"
)
...
func TestConfigureClient(t *testing.T) {
	...
	tests := []struct {
		...
		args     args
		wantErr  bool
		expected *modelexperiments.APIClient
	}{
		{
			...
			expected: func() *modelexperiments.APIClient {
				apiClient, err := modelexperiments.NewAPIClient(
		...
		},
		{
			...
			args: args{
				providerData: &core.ProviderData{
					Version:                        testVersion,
					ModelExperimentsCustomEndpoint: testCustomEndpoint,
				},
			},
			expected: func() *modelexperiments.APIClient {
				apiClient, err := modelexperiments.NewAPIClient(
