...
const (
	testVersion        = "1.2.3"
	testCustomEndpoint = "https://telemetrylink-custom-endpoint.api.stackit.cloud"
)
...
func TestConfigureClient(t *testing.T) {
	...
	tests := []struct {
		...
		args     args
		wantErr  bool
		expected *telemetrylink.APIClient
	}{
		{
			...
			expected: func() *telemetrylink.APIClient {
				apiClient, err := telemetrylink.NewAPIClient(
		...
		},
		{
			...
			args: args{
				providerData: &core.ProviderData{
					Version:                     testVersion,
					TelemetryLinkCustomEndpoint: testCustomEndpoint,
				},
			},
			expected: func() *telemetrylink.APIClient {
				apiClient, err := telemetrylink.NewAPIClient(
