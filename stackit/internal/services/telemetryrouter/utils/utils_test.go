...
const (
	testVersion        = "1.2.3"
	testCustomEndpoint = "https://telemetry-router-custom-endpoint.api.stackit.cloud"
)
...
func TestConfigureClient(t *testing.T) {
	...
	tests := []struct {
		...
		args     args
		wantErr  bool
		expected *telemetryrouter.APIClient
	}{
		{
			...
			expected: func() *telemetryrouter.APIClient {
				apiClient, err := telemetryrouter.NewAPIClient(
		...
		},
		{
			...
			args: args{
				providerData: &core.ProviderData{
					Version:                       testVersion,
					TelemetryRouterCustomEndpoint: testCustomEndpoint,
				},
			},
			expected: func() *telemetryrouter.APIClient {
				apiClient, err := telemetryrouter.NewAPIClient(
