...
var testConfigDestinationVarsOTLPBearerToken = config.Variables{
	...
	"config_filter_value0":  config.StringVariable("value1"),
	"config_filter_value1":  config.StringVariable("value2"),
	"token_display_name":    config.StringVariable("tf-acc-test-tlmr-dest"),
}
...
var testConfigDestinationVarsS3 = config.Variables{
	...
	"config_s3_bucket":                     config.StringVariable("tf-acc-test-tlmr-dest"),
}
		...
		Steps: []resource.TestStep{
			...
			// Datasource
			{
				...
				Check: resource.ComposeAggregateTestCheckFunc(
					...
					resource.TestCheckResourceAttrSet("data.stackit_telemetryrouter_destination.destination", "config.opentelemetry.uri"),
