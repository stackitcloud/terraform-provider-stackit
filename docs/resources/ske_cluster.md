...
- `max_surge` (Number) Maximum number of additional VMs that are created during an update. If set (larger than 0), then it must be at least the amount of zones configured for the nodepool. max_surge and max_unavailable can not be 0 at the same time, at least one of them need to be set (larger than 0).
- `max_unavailable` (Number) Maximum number of additional VMs that are created during an update. If set (larger than 0), then it must be at least the amount of zones configured for the nodepool. max_surge and max_unavailable can not be 0 at the same time, at least one of them need to be set (larger than 0).
