...
- `flavor_id` (String) The flavor ID of the PostgreSQL Flex instance. Can only be set when `flavor` and `replicas` are not set. You can list available flavors using the [STACKIT CLI](https://github.com/stackitcloud/stackit-cli):
```bash
